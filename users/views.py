...
class RequestPasswordResetView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Create the full reset link with both uidb64 and token
            ...
            return Response({"detail": "Reset email sent"}, status=status.HTTP_200_OK)
        except User.DoesNotExist:
...
class ResetPasswordView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        try:
            # Decode the uidb64 to get the user's primary key (pk)
            uid = force_str(urlsafe_base64_decode(uidb64))
            user = User.objects.get(pk=uid)
        except (TypeError, ValueError, OverflowError, User.DoesNotExist):
            return Response({"error": "Invalid password reset link. Please try again."}, status=status.HTTP_400_BAD_REQUEST)

        # Check if the provided token is valid for this user
        if not default_token_generator.check_token(user, token):
            return Response({"error": "Invalid or expired token. Please request a new password reset link."}, status=status.HTTP_400_BAD_REQUEST)
